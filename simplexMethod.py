...
# xb will contain the amount of resources 
b = np.array([8, 10, 15])		 
xb = np.transpose([b])
...
# [x1: 0, x2: 1, x3: 2, s1: 3, s2: 4, s3: 5] => objective function
B = np.array([[3], [4], [5]]) # slack varible index is stored in the B

# cb contains their corresponding coefficients in Z
cb = np.array(c[3]) # initially slack variables
for i in range(4, 6):
	cb = np.vstack((cb, c[i]))		 				 
...
print("Initial Table") 
print("B \tCB \tXB \tx1 \tx2 \tx3 \ts1 \ts2 \ts3") 
for row in table: 
	for el in row: 
